...
const err = fs.createWriteStream(path.join(vltFolder, 'err.txt'), {flags: 'w'});
...
function preprocess_loading_fragments(data) {
  ...
  if (fragment.maskMode == 'no_mask') {
    if (mirror) {
      python = spawn('python3', [path.join(pythonFolder, 'mirror_cut.py'), imageURL, "no_mask", vltFolder], {stdio: ['ignore', out, err]});
    }
    else python = spawn('python3', [path.join(pythonFolder, 'cut_image.py'), imageURL, "no_mask", vltFolder], {stdio: ['ignore', out, err]});
  } else if (fragment.maskMode == 'boundingbox') {
    if (mirror) {
      python = spawn('python3', [path.join(pythonFolder, 'mirror_cut.py'), imageURL, JSON.stringify(boxPoints), vltFolder], {stdio: ['ignore', out, err]});
    }
    else python = spawn('python3', [path.join(pythonFolder, 'cut_image.py'), imageURL, JSON.stringify(boxPoints), vltFolder], {stdio: ['ignore', out, err]});
  } else if (fragment.maskMode == 'polygon') {
    if (mirror) {
      python = spawn('python3', [path.join(pythonFolder, 'mirror_cut.py'), imageURL, JSON.stringify(polygonPoints), vltFolder], {stdio: ['ignore', out, err]});
    } else {
      python = spawn('python3', [path.join(pythonFolder, 'cut_image.py'), imageURL, JSON.stringify(polygonPoints), vltFolder], {stdio: ['ignore', out, err]});
    }
...
function filterImages(tableID, urls) {
  ...
  const python = spawn('python3', [path.join(pythonFolder, 'filter_images.py'), vltFolder, jsonPath], {stdio: ['ignore', out, err]});
  // python.stderr.pipe(process.stdout);
  // python.stdout.pipe(process.stdout);
  python.on('close', function(code) {
